...
game_tree get_middle(game_tree t)
{
	...
	t->root=get_t_node_middle(t->root);
...
game_tree get_right(game_tree t) 
{
	...
	t->root=get_t_node_right(t->root);
...
void set_left(game_tree t, game_tree l)
{
	...
	set_t_node_left( get_t_node_left( t->root),l->root);
...
void set_middle(game_tree t, game_tree m)
{
	...
	set_t_node_middle( get_t_node_middle(t->root),m->root);
...
void set_right(game_tree t, game_tree r)
{
	...
	set_t_node_right(get_t_node_right(t->root),r->root);
...
void generate_levelDF(game_tree t, stack k)
{
	...
	int currentrow=get_current_row(s);
	int currentcolumn=get_current_column(s);	
	for (int i=0;i<MOVE_COUNT-1;i++){
		n=clone(s);
		game_tree c;
		
		if (move(n,currentrow+HORIZ_MOVES[i],currentcolumn+VERT_MOVES[i])){
			printf("aaaaaaaaaaaaaaaaaaaaaaaaaaaaa\n");
			set_t_node_data(c->root,n);	
		//	c->root->data=n;	
			push(k,c);
		}	
	printf("wwwwwwwwwwwww\n");
	}
	printf("vccccccccccccccccccccccc\n");
...
game_tree build_gameDF(game_tree t, stack k)
{
	...
	game_state s = (game_state)(get_data(t));
	show_game_state(s);
...
/*
	...
	const int HORIZ_MOVES[] = { -1, 0, +1, 0 };	// moves left/right
	const int VERT_MOVES[] = { 0, -1, 0, +1 };	// moves up/down
	int currentrow=get_current_row(s)-1;
	int currentcolumn=get_current_column(s)-1;	
	for (int i=0;i<MOVE_COUNT;i++){
		game_state n=clone(s);
		game_tree c;
		printf("%d\n",currentcolumn+VERT_MOVES[i]);
		if (   move(n,currentrow+HORIZ_MOVES[i],currentcolumn+VERT_MOVES[i])){
			
		
			init_game_tree(&c, false, n);
			build_gameDF(c,k);
		//	push(k,c);
		}	
	//	printf("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaend\n");
