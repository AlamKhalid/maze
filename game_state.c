...
{
	trace("set_current_column: set_current_column starts");

	s->current_column=c;

	trace("set_current_column: set_current_column ends");
}
...
bool valid(game_state s, int r, int c)
{
	...
	if (get_value(s->board[r][c])==32|| get_value(s->board[r][c])==42){
	return true;
	}
	return false;
}
...
bool move(game_state s, int r, int c)
{
	...
	char msg[31];
	if (r-1<0 || r-1>=get_row_size(s) ){
		return false;	
	}
	if(c-1<0 || c-1>=get_column_size(s) ){
		return false;
	}
	
	trace("move: move begins");
	if (!get_visited(s->board[r-1][c-1])  &&  get_value(s->board[r-1][c-1])!=-1){
		printf("%d\n",get_value(s->board[r-1][c-1]));
		
		set_value(s->board[r-1][c-1],250);
...
bool solved(game_state s)
{
	...
	if (get_value(s->board[get_current_column(s)][get_current_row(s)])==s->goal){
		return true;
	}
	return false;
}
...
void show_game_state(game_state s)
{
	...
	for (int r = 0; r < s->row_size; r++)
	{
		...
		{
			// show square
			here = ((s->current_row - 1 == r) && (s->current_column - 1 == c));
